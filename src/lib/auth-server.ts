...
export async function getCurrentUser(): Promise<User | null> {
  ...
  const a = (await cookies()).get("auth_token")
  console.log(a, 10, "auth")
  ...
  try {
    ...
    const { status, data: user } = await response.json() as { status: number, data?: User, message?: string }
  
    console.log("Current user:", user, status)
